...
const ScoreBoard = styled.div`
  ...
  display: flex;
  width: 160px;
  box-shadow: ${({gameover}) => gameover ? "0 10px 5px -5px rgba(0,0,0,.2)" : "0 2px 0 0 rgba(0,0,0,.2)"};
...
export default ({score,gameover}) => {
  return (
    <ScoreBoard gameover={gameover}>
      ...
      <ScoreNum gameover={gameover} key={gameover ? "gameover" :score}>{score}</ScoreNum>
      {gameover?<Restart>press <span>start</span> to restart</Restart>:null}
    </ScoreBoard>
  )
}